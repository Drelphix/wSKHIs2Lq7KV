public class Application {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < data.points.length; i++) {
            ...
            data.gapsFNew.add(new Calculations().GetGap(data.points[i], data.steps, data.newton - 1));
        ...
        for (int i = 0; i < data.points.length; i++) {
            ...
            out.OutPower(data.points[i], data.arrLagranj[i], "F Лагранж");
            data.arrFLagranj[i] = calc.CalcLagranj(data.gapsFLag.get(i), data.lagranj - 1, data.points[i]);
            out.OutPower(data.points[i], data.arrFLagranj[i], "L Лагранж");
            data.arrNewton[i] = calc.CalcNewton(data.gapsNew.get(i), data.newton, data.points[i], data.step);
            out.OutPower(data.points[i], data.arrNewton[i], "F Ньютон");
            data.arrFNewton[i] = new Calculations().CalcNewton(data.gapsFNew.get(i), data.newton, data.points[i], data.step);
            new Out().OutPower(data.points[i], data.arrNewton[i], "N Ньютон");
        }
